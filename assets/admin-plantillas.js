...
function renderSelectorPlantillas() { const select = document.getElementById("selectorPlantillaRepositorio"); if (!select) return; const valorActual = plantillaActual?.id ? String(plantillaActual.id) : ""; select.innerHTML = ""; plantillasCache.forEach((plantilla) => { const option = document.createElement("option"); option.value = String(plantilla.id); const actividad = plantilla.actividad_titulo || "Sin actividad asociada"; option.textContent = `${plantilla.nombre || "Plantilla sin nombre"} · ${actividad}`; select.appendChild(option); }); if (!plantillasCache.length) { const option = document.createElement("option"); option.value = ""; option.textContent = "No hay plantillas guardadas"; select.appendChild(option); select.value = ""; select.disabled = true; return; } select.disabled = false; select.value = plantillasCache.some((item) => String(item.id) === valorActual) ? valorActual : String(plantillasCache[0].id); }
