...
export async function onRequestPost(context) {
  ...
  try {
    ...
    if (!version) {
      return json({ ok: false, error: "Debes indicar una versión válida del documento." }, 400);
    }

    if (documentoId) {
      const actual = await obtenerDocumentoObjetivo(env, session, documentoId);
      if (!actual) {
        return json({ ok: false, error: "No se encontró el documento seleccionado." }, 404);
      }
