    ...
    if (resumen === "1") {
      let sql = `
        SELECT
          ...
          COUNT(
            CASE
              WHEN r.estado IN ('PENDIENTE', 'EN_REVISION', 'PROVISIONAL', 'CONFIRMADA', 'SUSPENDIDA') THEN r.id
            END
          ) AS numero_reservas,
          COALESCE(SUM(
            CASE
              WHEN r.estado IN ('PENDIENTE', 'EN_REVISION', 'PROVISIONAL', 'CONFIRMADA', 'SUSPENDIDA') THEN r.personas
              ELSE 0
            END
          ), 0) AS ocupadas,
          (
